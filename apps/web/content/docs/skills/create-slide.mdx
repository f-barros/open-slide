...
4. **Authoring** — writes a modular deck (`index.tsx` + `slides/` +
   `components/`), deferring to
   [`/slide-authoring`](/docs/skills/slide-authoring) for the *how*.
...
A working `slides/<id>/index.tsx` that imports one page file per slide,
plus shared components under `components/` when needed. Drop into the dev

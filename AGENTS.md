...
| Path | Package | Role |
| --- | --- | --- |
| `packages/core` | `@comp-slide/core` | Runtime (viewer, present mode, inspector), Vite plugin, `open-slide` dev/build CLI. |
| `packages/cli` | `@comp-slide/cli` | `npx @comp-slide/cli init` scaffolder + project template. |
| `apps/demo` | private | Local consumer of `@comp-slide/core` via `workspace:*`. Dogfood target — run `pnpm dev` here to exercise the framework. |

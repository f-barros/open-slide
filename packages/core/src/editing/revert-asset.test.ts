...
describe('applyRevertAsset', () => {
  ...
  it('adds ImagePlaceholder to an existing @comp-slide/core named import', () => {
    const src = [
  ...
  it('adds a separate value import when the only @comp-slide/core import is type-only', () => {
    const src = [
    ...
    expect(r.source).toContain("import { ImagePlaceholder } from '@comp-slide/core';");
  });

  it('adds a fresh @comp-slide/core import when none exists', () => {
    const src = [
      "import hero from './assets/hero.png';",
      'export default [() => (',
      "  <img src={hero} alt='x' style={{ objectFit: 'cover' }} />",
    ...
    const r = applyRevertAsset(src, './assets/hero.png');
    if (!r.ok) throw new Error(`expected ok, got ${r.error}`);
    expect(r.source.split('\n')[0]).toBe("import { ImagePlaceholder } from '@comp-slide/core';");
  });
